...
const hasRisk = () => {
  ...
  const hasHighRisk = () => {
    return getTotalScore() >= 13 || answers["q9"] >= 1; // คำถามข้อ 9 เกี่ยวกับการทำร้ายตัวเอง
  };
  ...
  const renderQuestion = (question: any, index: number) => (
    <div key={question.id} className="bg-white rounded-2xl p-8 shadow-lg border border-gray-100 hover:shadow-xl transition-shadow duration-300">
      ...
      {/* เตือนพิเศษสำหรับคำถามข้อ 9 */}
      {question.id === "q9" && answers[question.id] >= 1 && (
        <div className="mt-6 p-4 bg-red-50 border-l-4 border-red-500 rounded-lg">
          <div className="flex items-center gap-3">
            <AlertCircle className="text-red-600 flex-shrink-0" size={20} />
            <div className="text-red-800">
              <p className="font-semibold">คำเตือนสำคัญ</p>
              <p className="text-sm mt-1">หากคุณมีความคิดทำร้ายตัวเอง กรุณาติดต่อสายด่วน 1323 หรือ 1300 ทันที</p>
            </div>
          </div>
        </div>
      )}
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-indigo-50 font-kanit">
      ...
      <div className="max-w-5xl mx-auto px-6 -mt-12 relative z-10">
        ...
        <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 mb-10 p-8 backdrop-blur-sm">
          ...
          {/* แสดงเตือนพิเศษหากมีความเสี่ยงสูง */}
          {hasHighRisk() && getCompletedQuestions() > 0 && (
            <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-xl">
              <div className="flex items-center gap-3">
                <AlertCircle className="text-red-600 flex-shrink-0" size={24} />
                <div className="text-red-800">
                  <p className="font-semibold">ต้องการความช่วยเหลือ</p>
                  <p className="text-sm mt-1">ผลการประเมินแสดงให้เห็นถึงความเสี่ยงที่ควรได้รับการดูแลจากผู้เชี่ยวชาญ</p>
                </div>
              </div>
            </div>
          )}
        ...
        {/* Emergency Contact */}
        <div className="bg-gradient-to-r from-red-600 to-orange-600 rounded-2xl p-6 mb-8 text-white shadow-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div className="bg-white/20 rounded-lg p-4">
              ...
              <div className="text-red-100">1323 (24 ชั่วโมง)</div>
            </div>
            <div className="bg-white/20 rounded-lg p-4">
              <div className="font-semibold mb-1">ศูนย์ช่วยเหลือ</div>
              <div className="text-red-100">1300 (24 ชั่วโมง)</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 p-8 mb-12">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-8 text-lg">
              {isFormComplete() 
                ? `คะแนนรวม: ${getTotalScore()} - ${currentRisk.level}`
                : `คุณได้ตอบคำถามไปแล้ว ${getCompletedQuestions()} จาก 9 ข้อ`
              }
            </p>
            ...
            >
              {isFormComplete() ? (
                ...
                <div className="flex items-center justify-center gap-3">
                  <AlertCircle size={24} />
                  <span>กรุณาตอบคำถามให้ครบ 9 ข้อ</span>
                </div>
              )}
            </button>
          </div>
