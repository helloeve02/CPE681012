...
export default function OtpPage() {
  ...
  const navigate = useNavigate(); // ✅ hook ของ react-router
  ...
  const handleVerifyOtp = async () => {
    try {
      ...
      if (res.ok) {
        setMessage("✅ OTP ถูกต้อง กำลังไปหน้าเมนู...");
        setTimeout(() => {
          navigate("/menu"); // ✅ redirect ไปหน้า /menu
        }, 1500);
      } else {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="bg-white p-8 rounded-2xl shadow-lg w-96">
        <h1 className="text-xl font-bold mb-4">🔐 รีเซ็ตรหัสผ่าน</h1>
        ...
        {step === 2 && (
          <>
            <label className="block mb-2">กรอก OTP</label>
            <input
              type="text"
              className="w-full border rounded-lg p-2 mb-4"
