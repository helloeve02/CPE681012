...
const ChooseAvoid = () => {
  ...
  const [activeTab, setActiveTab] = useState<"recommended" | "avoided">("recommended");
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [res] = await Promise.all([
          GetAllChooseAvoid(),
          delay(300), // ensures at least 300ms loading
        ]);
      ...
      {items.map((item, index) => (
        <div
          ...
          className={`
            ...
            ${isRecommended ? 'hover:shadow-green-200/50' : 'hover:shadow-red-200/50'}
        ...
        >
          ...
          <div className="p-3">
            ...
            `}>
              <div className={`w-2 h-2 rounded-full mr-1 ${isRecommended ? 'bg-green-400' : 'bg-red-400'}`}></div>
                  ...
                  >
                    ...
                    <span className="relative flex items-center">
                      <div className={`w-2 h-2 rounded-full mr-2 ${
                        activeTab === "recommended" ? 'bg-white' : 'bg-green-400'
                      }`}></div>
                      ควรทาน
                    </span>
                  </button>
          ...
          <div className="p-6 md:p-12 lg:px-20 xl:px-32 md:mx-25">
            <div className="max-w-7xl mx-auto space-y-8">
              {foodGroups.map((group, groupIndex) => {
                ...
                const itemsToShow = activeTab === "recommended" ? group.recommended : group.avoided;
                
                // Only render if the group has items for the current tab
                ...
                return (
                  <div
                    id={id}
                    key={group.topic}
                    className={`group relative ${isVisible ? 'animate-in slide-in-from-bottom-8 fade-in duration-700' : 'opacity-0'}`}
                  ...
                  >
                    ...
                    `}>
                      ...
                      <div className={`
                        absolute top-0 right-0 w-32 h-32 rounded-full opacity-10 transform translate-x-16 -translate-y-16
                        ${activeTab === "recommended" ? 'bg-green-400' : 'bg-red-400'}
                      `}></div>
                      <div className={`
                        absolute bottom-0 left-0 w-24 h-24 rounded-full opacity-10 transform -translate-x-12 translate-y-12
                        ${activeTab === "recommended" ? 'bg-emerald-400' : 'bg-rose-400'}
                      `}></div>
                      ...
                      <div className="relative">
                        <div className="flex items-center mb-6">
                          <div className={`
                            ...
                            ${activeTab === "recommended" ? 'bg-green-500' : 'bg-red-500'}
                          `}></div>
                        ...
                        <div className="mb-6">
                          ...
                          `}>
                            ...
                            {activeTab === "recommended" ? "ควรทาน" : "ควรเลี่ยง"}
                          </div>
                        </div>
                        
                        {renderFoodItems(itemsToShow, activeTab === "recommended")}
                      </div>
                    </div>
                  </div>
                );
